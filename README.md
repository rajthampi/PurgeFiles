...
Idea
Servers & PCs those accummulate large volume of log files, database export files consuming huge volume of storage require frequent house cleaning activities committed by administrators on regular intervals to free up the storage. Using this little tool, an administrator could schedule a task using Windows task scheduler to purge those files on regular intervals
